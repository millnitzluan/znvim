...
local H = require("plugins.highlights")
...
local function is_plain()
  return matches(M.ctx.filetype, plain_types.filetypes)
      or matches(M.ctx.buftype, plain_types.buftypes)
      or M.ctx.preview
end
...
local function get_diagnostics(seg_formatters_status)
  ...
  if vim.tbl_isempty(vim.lsp.get_active_clients({ bufnr = M.ctx.bufnr })) then
    return ""
  end
  ...
  for _, d in ipairs(diags) do
    if d.num > 0 then
      segments = fmt("%s %s", segments, seg(fmt("%s %s", d.sign, d.num), d.hl))
    end
  end
...
local function get_substitution_status()
  ...
  if not cool.status_with_icons() then
    return ""
  end
  -- local hl = ""
...
local function seg_git_symbol(truncate_at)
  if is_abnormal_buffer() or not is_valid_git() then
    return ""
  end
...
function _G.__statusline()
  ...
  if not is_focused() then
    return "%#StInactive# %F %m %r %= %{&spelllang} %y %8(%l,%c%) %8p%%"
  end
